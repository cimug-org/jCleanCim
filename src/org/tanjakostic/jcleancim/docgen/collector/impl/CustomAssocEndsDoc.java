 ...
 * class; applicable to meta-model classes only. For the layout, see {@link PropertiesDoc}.
 *
 * @author tatjana.kostic@ieee.org
 * @author laurent.guise@art-et-histoire.com
 * @version $Id: CustomAssocEndsDoc.java 27 2019-11-23 16:29:38Z dev978 $
 */
class CustomAssocEndsDoc extends AbstractPropertiesDoc {
	private static final Logger _logger = Logger.getLogger(CustomAssocEndsDoc.class.getName());

	public static final String INTRO_FMT = " shows all relations of %s with other classes.";
	public static final String CAPTION_FMT = "Relations of %s with other classes";

	/**
	 * Constructor.
	 */
	public CustomAssocEndsDoc(DocgenConfig docgenCfg, UmlClass c, BookmarkRegistry bmRegistry) {
		super(docgenCfg, c, "ae", String.format(INTRO_FMT, c.getName()),
				String.format(CAPTION_FMT, c.getName()), TableSpec.CUSTOM_ASSOC_ENDS, null,
				bmRegistry);
		...
		for (UmlAssociationEndPair aePair : retainedNatives) {
			...
			// String cond = get61850PresenceCondition(otherEnd);
			 ...
			 */
			// CellText desc = deduceCellText(getDeprecatedTextAsPrefix(otherEnd), otherEnd);

			CellText desc = deduceCellText(getDeprecatedTextAsPrefix(otherEnd),
					otherEnd.getDescription(), null, otherEnd);

			EntryDocImpl entry = EntryDocImpl.createData(bookmarkID, desc.formatInfo, name, mult,
					type, desc.text);
			addEntry(entry);
		...
		for (UmlAssociationEndPair aePair : retainedInheriteds) {
			...
			String type = prepareForHyperlink(otherEnd.getType());
			// String cond = get61850PresenceCondition(otherEnd);
			String description = getDeprecatedTextAsPrefix(otherEnd) + INHERITED_FROM
					+ prepareForHyperlink(myEnd.getType());

			EntryDocImpl entry = EntryDocImpl.createData(bookmarkID, null, name, mult, type,
					description);
			addEntry(entry);
	...
	private static void initAssocEndRawData(EntryDocImpl entry, UmlAssociationEnd otherEnd,
		...
		// entry.putCell(WAX.A_presCond, get61850PresenceCondition(otherEnd));
		// entry.addRawField(WAX.A_presCondArgs, "FIXME!");
	}

	// /** Returns presence condition for association ends. */
	// protected static final String get61850PresenceCondition(UmlAssociationEnd ae) {
	// return /* (ae.isConditional()) ? ae.getConditionNames() : */
	// ae.getMultiplicity().isOptional() ? UmlMultiplicity.Kind.O.toString()
