...
*   [What's new in this release](ReleaseNotes.md#release-notes-for-jcleancim-2.3.0) 
...
jCleanCim is available in two distributions, depending on how you want to use it. For each of these distribution types you have chose either a 32-bit or 64-bit option.  Which you choose depends upon the EA model file type you want to process (i.e. 32-bit `.eap`/`.eapx` project files or 64-bit `.qea`/`.qeap` project files from EA 16.0 and later.
...
|--------------|-----------|---------------|------------
| Binary distribution jCleanCim-2.4.0-x32-bin.zip | Run jCleanCim from the console (cmd.exe). | <ul><li>[32-bit Java 17+](https://www.azul.com/downloads/?version=java-17-lts&os=windows&architecture=x86-32-bit&package=jre#zulu) Java Runtime Environment (JRE). To verify whether you already have it installed, open the console window and type `java -version` <li>Enterprise Architect 15.x and earlier<li> (optional: to run doc generation) MS Word 2003+</ul> | Unzip the distribution anywhere on your file system; it will uncompress in its own directory tagged with the version so there is no danger of overwriting an older installation. For example, using WinZip, select "Extract to here" command. |
| Source distribution jCleanCim-2.4.0-x32-src.zip | Run jCleanCim from the console (cmd.exe) or from within eclipse.<br/><br/>Develop and build it with Apache ant or with eclipse. | <ul><li>[32-bit Java 17+](https://www.azul.com/downloads/?version=java-17-lts&os=windows&architecture=x86-32-bit&package=jdk#zulu) Java Development Kit (JDK). To verify whether you already have it installed, open the console window and type `javac -version` <li>Something to compile the code and create executable, e.g.:<ul><li>Apache [Ant 1.10.15+](https://dlcdn.apache.org//ant/binaries/apache-ant-1.10.15-bin.zip). To verify whether you already have it installed, open the console window and type `ant -version`. <li>or an IDE if you are already a Java developer.</ul> <li>Enterprise Architect 15.x and earlier<li>(optional, runtime: to run doc generation) MS Word 2003+<li>(optional, during ant build: to build javadoc with UML for any distribution, and to document ant build targets dependencies graph) [GraphViz application](https://www.graphviz.org/download/). To verify whether you already have it installed, open the console window and type `dot -version`</ul> | Same as for binary distribution (jCleanCim-[version]-[os]-bin.zip). <br/><br/> If used with eclipse, start eclipse SDK and after unzipping, use Import ->Existing project and browse to the unzipped directory. <br/><br/> Note: This is the most flexible option if you are developing, as you can have the eclipse project anywhere on your disk (not necessarily in an eclipse workspace). |
| Binary distribution jCleanCim-2.4.0-x64-bin.zip | Run jCleanCim from the console (cmd.exe). | <ul><li>[64-bit Java 17+](https://www.azul.com/downloads/?version=java-17-lts&os=windows&architecture=x86-64-bit&package=jre#zulu) Java Runtime Environment (JRE). To verify whether you already have it installed, open the console window and type `java -version` <li>Enterprise Architect 16.0+<li> (optional: to run doc generation) MS Word 2003+</ul> | Unzip the distribution anywhere on your file system; it will uncompress in its own directory tagged with the version so there is no danger of overwriting an older installation. For example, using WinZip, select "Extract to here" command. |
| Source distribution jCleanCim-2.4.0-x64-src.zip | Run jCleanCim from the console (cmd.exe) or from within eclipse.<br/><br/>Develop and build it with Apache ant or with eclipse. | <ul><li>[64-bit Java 17+](https://www.azul.com/downloads/?version=java-17-lts&os=windows&architecture=x86-64-bit&package=jdk#zulu) Java Development Kit (JDK). To verify whether you already have it installed, open the console window and type `javac -version` <li>Something to compile the code and create executable, e.g.:<ul><li>Apache [Ant 1.10.15+](https://dlcdn.apache.org//ant/binaries/apache-ant-1.10.15-bin.zip). To verify whether you already have it installed, open the console window and type `ant -version`. <li>or an IDE if you are already a Java developer.</ul> <li>Enterprise Architect 16.0+<li>(optional, runtime: to run doc generation) MS Word 2003+<li>(optional, during ant build: to build javadoc with UML for any distribution, and to document ant build targets dependencies graph) [GraphViz application](https://www.graphviz.org/download/). To verify whether you already have it installed, open the console window and type `dot -version`</ul> | Same as for binary distribution (jCleanCim-[version]-[os]-bin.zip). <br/><br/> If used with eclipse, start eclipse SDK and after unzipping, use Import ->Existing project and browse to the unzipped directory. <br/><br/> Note: This is the most flexible option if you are developing, as you can have the eclipse project anywhere on your disk (not necessarily in an eclipse workspace). |
